...
// =============================================================================
//
// Milestone 11: Rate-Limited / Transport-Limited Tangential State
//
// This demo extends milestone 10 with:
//   1. Transport magnitude clamp (方案 A): limit |xi_transport| to max allowed
//   2. Rotation-aware attenuation (方案 B): decay transport based on normal change angle
//   3. Diagnostics output for raw vs limited transport norms
//
// Scene: Case A (rotational contact) and Case B (multi-patch)
//
// Output:
//   out/milestone_11/sdf_patch_transport_limited_case_A.csv
//   out/milestone_11/sdf_patch_transport_limited_case_B.csv
//   out/milestone_11/sdf_patch_transport_limited_compare.csv
//   out/milestone_11/sdf_patch_transport_diagnostics.csv
// =============================================================================
...
// =============================================================================
// Persistent patch track with transport-limited tangential state
// =============================================================================
...
struct PersistentPatchTrack {
    ...
    double xi1;
    double xi2;
    // Legacy: 3D tangential displacement (for backward compatibility)
...
enum class ContactMode {
    ...
    PatchStickSlip,
    PatchFrameConsistent,
    PatchTransportLimited // NEW: transport-limited tangential state
};
...
struct CaseConfig {
    ...
    double ema_alpha;
    // Transport limit parameters
    double transport_max_norm;       // xi_max for clamp
    double transport_attenuation_beta; // beta for exp(-beta * theta)
    double transport_carry_alpha;    // alpha for partial carry-over
};
...
// =============================================================================
// Transport-limited tangential state (Milestone 11)
// =============================================================================

struct TransportLimitedResult {
    double xi1_new;
    double xi2_new;
    double raw_transport_norm;      // |xi_projected| before limits
    double limited_transport_norm;  // |xi_projected| after limits
    double transport_attenuation;   // attenuation factor applied
    int transport_clamp_hit;        // 1 if clamp was activated
    double frame_rotation_angle;
};

TransportLimitedResult TransportLimitedTangentialState(
    ...
    double xi_max_norm,        // clamp threshold (方案 A)
    double attenuation_beta,   // rotation-aware decay (方案 B)
    double carry_alpha         // partial carry-over factor (方案 C)
) {
    TransportLimitedResult result;
    ...
    // Step 3: Apply transport limits
    
    // 方案 B: Rotation-aware attenuation
    double theta = result.frame_rotation_angle;
    double attenuation = std::exp(-attenuation_beta * theta);
    ...
    if (xi_proj_norm > xi_max_norm) {
        xi_projected = xi_projected * (xi_max_norm / xi_proj_norm);
        result.transport_clamp_hit = 1;
    }
    result.limited_transport_norm = xi_projected.Length();

    // Step 4: Partial carry-over (方案 C)
    xi_projected *= carry_alpha;
    result.limited_transport_norm = xi_projected.Length();

    // Step 5: Represent in new local basis
...
std::vector<PatchPrimitive> BuildPatches(
    ...
    for (size_t c = 0; c < components.size(); c++) {
        ...
        patch.raw_transport_norm = 0.0;
        patch.limited_transport_norm = 0.0;
        patch.transport_attenuation = 1.0;
        patch.transport_clamp_hit = 0;
        ...
        // Estimate patch radius from area (assuming circular patch)
...
// =============================================================================
// Compute patch constitutive force (normal only)
// =============================================================================
...
// =============================================================================
// Compute patch frame-consistent force (milestone 10)
// =============================================================================
...
FrameConsistentResult ComputePatchFrameConsistentForce(
    ...
    if (prev_track != nullptr) {
        ...
        // Standard transport (no limits)
        ChVector3d xi_world = xi1_old * prev_track->tangent_t1 + xi2_old * prev_track->tangent_t2;
        double normal_component = xi_world.Dot(patch.normal);
        ChVector3d xi_projected = xi_world - normal_component * patch.normal;

        result.xi1 = xi_projected.Dot(patch.tangent_t1);
        result.xi2 = xi_projected.Dot(patch.tangent_t2);
        result.transported_state_norm = xi_projected.Length();
        result.frame_rotation_angle = std::acos(std::max(-1.0, std::min(1.0, prev_track->normal.Dot(patch.normal))));
...
// =============================================================================
// Compute patch transport-limited force (NEW - milestone 11)
// =============================================================================

struct TransportLimitedForceResult {
    ChVector3d force;
    double xi1;
    double xi2;
    StickSlipState state;
...
TransportLimitedForceResult ComputePatchTransportLimitedForce(
    const PatchPrimitive& patch,
    const ChVector3d& patch_center_velocity,
    double k_patch,
    double c_patch,
    ...
    TransportLimitedForceResult result;
    result.xi1 = 0.0;
    result.xi2 = 0.0;
    result.raw_transport_norm = 0.0;
    result.limited_transport_norm = 0.0;
    ...
    // Initialize or transport-limited tangential state
    if (prev_track != nullptr) {
        TransportLimitedResult transport = TransportLimitedTangentialState(
        ...
        result.raw_transport_norm = transport.raw_transport_norm;
        result.limited_transport_norm = transport.limited_transport_norm;
        result.transport_attenuation = transport.transport_attenuation;
        result.transport_clamp_hit = transport.transport_clamp_hit;
        result.frame_rotation_angle = transport.frame_rotation_angle;
        result.state = prev_track->stick_slip_state;
    } else {
        result.xi1 = 0.0;
        result.xi2 = 0.0;
...
CaseResult RunCaseMode(
    ...
    std::vector<double> transport_attenuations;
    ...
    while (sys.GetChTime() < config.total_time) {
        ...
        if (mode == ContactMode::PatchForceSum || mode == ContactMode::PatchConstitutive || 
            ...
            mode == ContactMode::PatchFrameConsistent || mode == ContactMode::PatchTransportLimited) {
            ...
            for (size_t pi = 0; pi < patches.size(); pi++) {
                ...
                if (mode == ContactMode::PatchForceSum) {
                    ...
                    // Placeholder - same as milestone 8
                    patch.force = ComputePatchConstitutiveForceNormal(
                        patch, patch_center_vel, config.stiffness, config.damping
                    );
                    patch.torque = (patch.center - body_pos).Cross(patch.force);
                } else if (mode == ContactMode::PatchStickSlip) {
                    // Placeholder - milestone 9 stick-slip (simplified)
                    patch.force = ComputePatchConstitutiveForceNormal(
                        patch, patch_center_vel, config.stiffness, config.damping
                    );
                    patch.torque = (patch.center - body_pos).Cross(patch.force);
                    ...
                    // Milestone 10 frame-consistent (no limits)
                    ...
                    patch.torque = (patch.center - body_pos).Cross(patch.force);
                    patch.raw_transport_norm = fc_result.transported_state_norm;
                    patch.limited_transport_norm = fc_result.transported_state_norm;
                    patch.transport_attenuation = 1.0;
                    patch.transport_clamp_hit = 0;
                    ...
                    for (auto& track : current_tracks) {
                        if (track.current_frame_patch_id == patch.patch_id) {
                            ...
                            if (fc_result.state == StickSlipState::Stick) track.stick_steps++;
                            else track.slip_steps++;
                            if (prev_track != nullptr && prev_track->stick_slip_state == StickSlipState::Stick && fc_result.state == StickSlipState::Slip) {
                ...
                } else if (mode == ContactMode::PatchTransportLimited) {
                    // Milestone 11 transport-limited
                    TransportLimitedForceResult tl_result = ComputePatchTransportLimitedForce(
                        patch, patch_center_vel, config.stiffness, config.damping,
                        config.tangential_stiffness, config.tangential_damping,
                        config.friction_coefficient, config.time_step, prev_track,
                        config.transport_max_norm,
                        config.transport_attenuation_beta,
                        config.transport_carry_alpha
                    );

                    patch.force = tl_result.force;
                    patch.stick_slip_state = tl_result.state;
                    patch.normal_force_magnitude = tl_result.normal_force_magnitude;
                    patch.tangential_force_magnitude = tl_result.tangential_force_magnitude;
                    patch.torque = (patch.center - body_pos).Cross(patch.force);
                    patch.raw_transport_norm = tl_result.raw_transport_norm;
                    patch.limited_transport_norm = tl_result.limited_transport_norm;
                    patch.transport_attenuation = tl_result.transport_attenuation;
                    patch.transport_clamp_hit = tl_result.transport_clamp_hit;
                    patch.frame_rotation_angle = tl_result.frame_rotation_angle;
                    patch.tangential_displacement_local_norm = std::sqrt(tl_result.xi1 * tl_result.xi1 + tl_result.xi2 * tl_result.xi2);

                    for (auto& track : current_tracks) {
                        if (track.current_frame_patch_id == patch.patch_id) {
                            track.xi1 = tl_result.xi1;
                            track.xi2 = tl_result.xi2;
                            track.stick_slip_state = tl_result.state;
                            if (tl_result.state == StickSlipState::Stick) track.stick_steps++;
                            else track.slip_steps++;
                            if (prev_track != nullptr && prev_track->stick_slip_state == StickSlipState::Stick && tl_result.state == StickSlipState::Slip) {
                                track.stick_to_slip_transitions++;
                            }
                            track.accumulated_transport_norm += tl_result.limited_transport_norm;
                            track.total_transport_clamp_hits += tl_result.transport_clamp_hit;
                            break;
                        }
                    }
                }
        ...
        if (mode == ContactMode::PatchFrameConsistent || mode == ContactMode::PatchTransportLimited) {
            for (const auto& track : active_tracks) {
                ...
                if (mode == ContactMode::PatchTransportLimited) {
                    total_transport_clamp_hits += track.total_transport_clamp_hits;
                }
            }
        }
    }
...
int main(int argc, char* argv[]) {
    std::cout << "=== Milestone 11: Transport-Limited Tangential State ===" << std::endl;

    // -- Output directory --
    std::string project_root = GetProjectRoot();
    std::string out_dir = project_root + "/out/milestone_11";
    EnsureDir(out_dir);

    // ========================================================================
    // Milestone 10 Baseline Reference
    // ========================================================================
    std::cout << "\n=== Milestone 10 Baseline Reference ===" << std::endl;
    std::cout << "Case A (Rotational Contact):" << std::endl;
    std::cout << "  FrameConsistent: y_error=0.553m, torque_z=+2.30Nm, transported_norm=5.62m, stable=NO" << std::endl;
    std::cout << "Case B (Multi-Patch):" << std::endl;
    std::cout << "  FrameConsistent: y_error=0.029m, torque_z=-0.023Nm, transported_norm=1.96m, stable=YES" << std::endl;
    ...
    // Transport limit parameters
    case_A.transport_max_norm = 0.01;         // 1cm max transport
    case_A.transport_attenuation_beta = 5.0;  // Strong attenuation for rotation
    case_A.transport_carry_alpha = 0.5;       // 50% carry-over
    ...
    // Transport limit parameters (same as Case A)
    case_B.transport_max_norm = 0.01;
    case_B.transport_attenuation_beta = 5.0;
    case_B.transport_carry_alpha = 0.5;
    ...
    std::vector<ContactMode> modes = {
        ...
        ContactMode::PatchTransportLimited
    };
    std::vector<std::string> mode_names = {"Pointwise", "PatchForceSum", "PatchConstitutive", "PatchFrameConsistent", "PatchTransportLimited"};
    ...
    for (const auto& config : cases) {
        ...
        std::cout << "Transport limits: xi_max=" << config.transport_max_norm 
                  << ", beta=" << config.transport_attenuation_beta 
                  << ", alpha=" << config.transport_carry_alpha << std::endl;
        ...
        for (size_t mi = 0; mi < modes.size(); mi++) {
            ...
            if (mi == 3 || mi == 4) {
                ...
                std::cout << "    Avg Raw Transport Norm: " << result.avg_raw_transport_norm << " m" << std::endl;
                std::cout << "    Avg Frame Rotation Angle: " << result.avg_frame_rotation_angle << " rad" << std::endl;
                if (mi == 4) {
                    std::cout << "    Transport Clamp Hits: " << result.total_transport_clamp_hits << std::endl;
                }
            }
        }
    ...
    // Case B output
    std::ofstream case_B_file(out_dir + "/sdf_patch_transport_limited_case_B.csv");
    ...
    // Comparison
    std::ofstream compare_file(out_dir + "/sdf_patch_transport_limited_compare.csv");
    compare_file << "metric,case_A_pointwise,case_A_patch_constitutive,case_A_frame_consistent,case_A_transport_limited,case_B_pointwise,case_B_patch_constitutive,case_B_frame_consistent,case_B_transport_limited" << std::endl;
    ...
    compare_file << "avg_raw_transport_norm,0,0," << all_results[0][3].avg_raw_transport_norm << "," << all_results[0][4].avg_raw_transport_norm << ",0,0," << all_results[1][3].avg_raw_transport_norm << "," << all_results[1][4].avg_raw_transport_norm << std::endl;
    compare_file << "total_transport_clamp_hits,0,0,0," << all_results[0][4].total_transport_clamp_hits << ",0,0,0," << all_results[1][4].total_transport_clamp_hits << std::endl;
    compare_file.close();

    // Diagnostics output
    std::ofstream diag_file(out_dir + "/sdf_patch_transport_diagnostics.csv");
    diag_file << "metric,case_A_frame_consistent,case_A_transport_limited,case_B_frame_consistent,case_B_transport_limited" << std::endl;
    diag_file << "avg_raw_transport_norm,"
              << std::fixed << std::setprecision(6)
              << all_results[0][3].avg_raw_transport_norm << "," << all_results[0][4].avg_raw_transport_norm << ","
              << all_results[1][3].avg_raw_transport_norm << "," << all_results[1][4].avg_raw_transport_norm << std::endl;
    diag_file << "avg_frame_rotation_angle,"
              << all_results[0][3].avg_frame_rotation_angle << "," << all_results[0][4].avg_frame_rotation_angle << ","
              << all_results[1][3].avg_frame_rotation_angle << "," << all_results[1][4].avg_frame_rotation_angle << std::endl;
    diag_file << "total_transport_clamp_hits,0," << all_results[0][4].total_transport_clamp_hits << ",0," << all_results[1][4].total_transport_clamp_hits << std::endl;
    diag_file.close();
    ...
    std::cout << std::left << std::setw(18) << "Metric"
              << std::setw(18) << "PatchConstit"
              << std::setw(18) << "FrameConsistent"
              << std::setw(18) << "TransportLimited" << std::endl;
    std::cout << std::string(72, '-') << std::endl;
    std::cout << std::left << std::setw(18) << "Final Y (m)"
              << std::fixed << std::setprecision(6)
              << std::setw(18) << all_results[0][2].final_y
              << std::setw(18) << all_results[0][3].final_y
              << std::setw(18) << all_results[0][4].final_y << std::endl;
    std::cout << std::left << std::setw(18) << "Y Error (m)"
              << std::setw(18) << all_results[0][2].y_error
              << std::setw(18) << all_results[0][3].y_error
              << std::setw(18) << all_results[0][4].y_error << std::endl;
    std::cout << std::left << std::setw(18) << "Force Std (N)"
              << std::setw(18) << all_results[0][2].force_std_dev
              << std::setw(18) << all_results[0][3].force_std_dev
              << std::setw(18) << all_results[0][4].force_std_dev << std::endl;
    std::cout << std::left << std::setw(18) << "Avg Torque Z (Nm)"
              << std::setw(18) << all_results[0][2].avg_torque_z
              << std::setw(18) << all_results[0][3].avg_torque_z
              << std::setw(18) << all_results[0][4].avg_torque_z << std::endl;
    std::cout << std::left << std::setw(18) << "Avg Raw Transport"
              << std::setw(18) << 0.0
              << std::setw(18) << all_results[0][3].avg_raw_transport_norm
              << std::setw(18) << all_results[0][4].avg_raw_transport_norm << std::endl;
    std::cout << std::left << std::setw(18) << "Clamp Hits"
              << std::setw(18) << 0
              << std::setw(18) << 0
              << std::setw(18) << all_results[0][4].total_transport_clamp_hits << std::endl;
    std::cout << std::left << std::setw(18) << "Stable"
              << std::setw(18) << (all_results[0][2].is_stable ? "YES" : "NO")
              << std::setw(18) << (all_results[0][3].is_stable ? "YES" : "NO")
              << std::setw(18) << (all_results[0][4].is_stable ? "YES" : "NO") << std::endl;
    ...
    std::cout << std::left << std::setw(18) << "Final Y (m)"
              << std::fixed << std::setprecision(6)
              << std::setw(18) << all_results[1][2].final_y
              << std::setw(18) << all_results[1][3].final_y
              << std::setw(18) << all_results[1][4].final_y << std::endl;
    std::cout << std::left << std::setw(18) << "Y Error (m)"
              << std::setw(18) << all_results[1][2].y_error
              << std::setw(18) << all_results[1][3].y_error
              << std::setw(18) << all_results[1][4].y_error << std::endl;
    std::cout << std::left << std::setw(18) << "Force Std (N)"
              << std::setw(18) << all_results[1][2].force_std_dev
              << std::setw(18) << all_results[1][3].force_std_dev
              << std::setw(18) << all_results[1][4].force_std_dev << std::endl;
    std::cout << std::left << std::setw(18) << "Avg Torque Z (Nm)"
              << std::setw(18) << all_results[1][2].avg_torque_z
              << std::setw(18) << all_results[1][3].avg_torque_z
              << std::setw(18) << all_results[1][4].avg_torque_z << std::endl;
    std::cout << std::left << std::setw(18) << "Avg Raw Transport"
              << std::setw(18) << 0.0
              << std::setw(18) << all_results[1][3].avg_raw_transport_norm
              << std::setw(18) << all_results[1][4].avg_raw_transport_norm << std::endl;
    std::cout << std::left << std::setw(18) << "Clamp Hits"
              << std::setw(18) << 0
              << std::setw(18) << 0
              << std::setw(18) << all_results[1][4].total_transport_clamp_hits << std::endl;
    std::cout << std::left << std::setw(18) << "Stable"
              << std::setw(18) << (all_results[1][2].is_stable ? "YES" : "NO")
              << std::setw(18) << (all_results[1][3].is_stable ? "YES" : "NO")
              << std::setw(18) << (all_results[1][4].is_stable ? "YES" : "NO") << std::endl;

    // ========================================================================
    // Verification
    // ========================================================================

    bool case_a_transport_better = all_results[0][4].y_error < all_results[0][3].y_error;
    bool case_a_transport_norm_reduced = all_results[0][4].avg_raw_transport_norm < all_results[0][3].avg_raw_transport_norm;
    bool case_b_transport_stable = all_results[1][4].is_stable;
    bool case_b_not_degraded = all_results[1][4].y_error < all_results[1][0].y_error;

    std::cout << "\n=== Verification ===" << std::endl;
    std::cout << "  Case A transport-limited y_error vs frame-consistent: " << std::fixed << std::setprecision(4) 
              << all_results[0][4].y_error << " vs " << all_results[0][3].y_error << std::endl;
    std::cout << "  Case A improvement: " << (case_a_transport_better ? "PASS" : "FAIL") << std::endl;
    std::cout << "  Case A transport norm reduced: " << (case_a_transport_norm_reduced ? "PASS" : "FAIL") << std::endl;
    std::cout << "  Case B transport-limited stable: " << (case_b_transport_stable ? "PASS" : "FAIL") << std::endl;
    std::cout << "  Case B not degraded vs pointwise: " << (case_b_not_degraded ? "PASS" : "FAIL") << std::endl;

    bool pass = case_a_transport_better && case_a_transport_norm_reduced && case_b_transport_stable && case_b_not_degraded;
    std::cout << "  Overall: " << (pass ? "PASS" : "FAIL") << std::endl;

    std::cout << "\n=== Output Files ===" << std::endl;
    std::cout << "  " << out_dir + "/sdf_patch_transport_limited_case_A.csv" << std::endl;
    std::cout << "  " << out_dir + "/sdf_patch_transport_limited_case_B.csv" << std::endl;
    std::cout << "  " << out_dir + "/sdf_patch_transport_limited_compare.csv" << std::endl;
    std::cout << "  " << out_dir + "/sdf_patch_transport_diagnostics.csv" << std::endl;
