...
// =============================================================================
//
// Milestone 31: Chrono-original mesh contact baseline.
//
// This headless benchmark extracts the triangle-mesh contact pattern from
// Chrono's original MBS examples and turns it into a deterministic regression
// scene.  Each scenario loads one Chrono data OBJ used by the original demos
// and reuses that exact mesh for two backends:
...
//
// The output is aligned per frame and includes contact/patch counts,
// force/torque jumps, displacement/velocity/acceleration tracking, Coulomb
// ratio, and stability ratios suitable for regression checks.
//
// Outputs:
//   out/milestone_31/chrono_original_mesh_field_frames.csv
//   out/milestone_31/chrono_original_mesh_native_frames.csv
//   out/milestone_31/chrono_original_mesh_comparison.csv
//   out/milestone_31/chrono_original_mesh_geometry.csv
//
// =============================================================================
...
int main() {
    std::cout << "Milestone 31: Chrono-original mesh/OpenVDB field baseline" << std::endl;
    openvdb::initialize();

    const std::string project_root = GetProjectRoot();
    const std::filesystem::path out_dir = std::filesystem::path(project_root) / "out" / "milestone_31";
    std::filesystem::create_directories(out_dir);

    std::ofstream field_frames(out_dir / "chrono_original_mesh_field_frames.csv");
    std::ofstream native_frames(out_dir / "chrono_original_mesh_native_frames.csv");
    std::ofstream comparison(out_dir / "chrono_original_mesh_comparison.csv");
    std::ofstream geometry(out_dir / "chrono_original_mesh_geometry.csv");
    ...
    geometry << "scenario,source_demo,target_obj,target_vertices,target_faces,"
                "target_min_x,target_min_y,target_min_z,target_max_x,target_max_y,target_max_z,"
                "target_size_x,target_size_y,target_size_z,moving_samples,probe_radius,voxel_size,"
                "active_voxels,native_mesh_radius\n";

    const auto data_dir = std::filesystem::path(project_root) / "data" / "models";

    std::vector<ScenarioConfig> scenarios = {
        {"chrono_original_collision_trimesh_shoe_view_slide",
         data_dir / "bulldozer" / "shoe_view.obj",
         "src/demos/mbs/demo_MBS_collision_trimesh.cpp",
         ChVector3d(-0.035, 0.128, -0.060),
         ChVector3d(0.260, 0.128, 0.060),
         0.055,
         0.022,
         1.25,
         1.25,
         ...
         0.0020,
         0.0},
        {"chrono_original_tracks_shoe_collision_lateral_entry",
         data_dir / "bulldozer" / "shoe_collision.obj",
         "src/demos/mbs/demo_MBS_tracks.cpp",
         ChVector3d(0.100, 0.126, -0.205),
         ChVector3d(0.155, 0.126, 0.205),
         0.055,
         0.020,
         1.75,
         1.35,
    ...
    for (const auto& scenario : scenarios) {
        ...
        geometry << scenario.name << "," << scenario.source_demo << "," << scenario.target_obj.generic_string() << ","
                 << scene.target_mesh.vertices.size() << "," << scene.target_mesh.faces.size() << ","
                 << scene.target_bounds.min.x() << "," << scene.target_bounds.min.y() << ","
                 << scene.target_bounds.min.z() << "," << scene.target_bounds.max.x() << ","
                 << scene.target_bounds.max.y() << "," << scene.target_bounds.max.z() << ","
                 << scene.target_bounds.size.x() << "," << scene.target_bounds.size.y() << ","
                 << scene.target_bounds.size.z() << "," << scene.moving_graph.samples.size() << ","
                 << scenario.probe_radius << "," << scene.sdf.voxel_size << ","
    ...
    std::cout << "  " << (out_dir / "chrono_original_mesh_field_frames.csv").string() << std::endl;
    std::cout << "  " << (out_dir / "chrono_original_mesh_native_frames.csv").string() << std::endl;
    std::cout << "  " << (out_dir / "chrono_original_mesh_comparison.csv").string() << std::endl;
    std::cout << "  " << (out_dir / "chrono_original_mesh_geometry.csv").string() << std::endl;
    return 0;
}
